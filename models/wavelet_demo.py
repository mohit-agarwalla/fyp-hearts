...
class WaveletModel():
    ...
    def fit(self, X_train, y_train, demographic_train, X_val,y_val, demographic_val):
        ...
        if type(demographic_val) != bool:
            XF_train = np.concatenate((XF_train, np.array(demographic_train)), axis=1)
            XF_val = np.concatenate((XF_val, np.array(demographic_val)), axis=1)
    ...
    def predict(self, X, demographic_test):
        ...
        if type(demographic_test) != bool:
            XF = np.concatenate((XF, np.array(demographic_test)), axis=1)
