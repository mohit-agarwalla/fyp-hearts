...
class InceptionBlock1d(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.bn_relu(cat([c(bottled) for c in self.convs]+[self.conv_bottle(x)], dim=1))
        return out
...
class Inception1DClass(nn.Module):
...
def Inception1D(**kwargs):
    # COnstruct Inception mode
    return Inception1DClass(**kwargs)
